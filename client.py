...
# функциональность: детектирование сжатия/разжатия только профиля ладони, нахождене центра сжатой ладони
# новая фича: передачи многих точек ладони сразу
    ...
    with mp_hands.Hands(
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks != None:
                ...
                data_cube = {
                    'x21': int(x_centre_cube),
                    'y21': int(y_centre_cube),
                    'z21': int(z_palm),
                    'isClenched': is_close # 0 - open, 1 - close
                }

                for i in range(21):
                    data_cube['x' + str(i)] = int(coords_vr_x[i])
                    data_cube['y' + str(i)] = int(coords_vr_y[i])
                    data_cube['z' + str(i)] = int(z_palm)

            try:
                print("Sent: ", data_cube)
                await websocket.send(json.dumps(data_cube))
